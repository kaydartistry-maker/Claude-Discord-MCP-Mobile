...
The tool accepts a `voice` parameter to select between configured voices. Voices are configured via the `VOICE_MAP` secret — a JSON object mapping voice names to ElevenLabs voice IDs, so your IDs never live in the repo:

```bash
npx wrangler secret put VOICE_MAP
# paste: {"myvoice":"your_elevenlabs_voice_id","othervoice":"another_id"}
```
