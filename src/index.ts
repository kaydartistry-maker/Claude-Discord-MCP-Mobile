...
interface Env {
  ...
  // JSON of voice name -> ElevenLabs voice ID, e.g. {"myvoice":"abc123"}.
  // Set via `wrangler secret put VOICE_MAP` — keeps voice IDs out of the repo.
  VOICE_MAP?: string;
}
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        voice: { type: 'string', description: 'Voice name from your configured VOICE_MAP (defaults to the first configured voice)' },
      },
...
function getVoiceMap(env: Env): Record<string, string> {
  try {
    return env.VOICE_MAP ? JSON.parse(env.VOICE_MAP) : {};
  } catch {
    return {};
  }
}
  ...
  try {
    switch (name) {
      ...
      case 'discord_send_voice': {
        const voices = getVoiceMap(env);
        const voiceNames = Object.keys(voices);
        if (voiceNames.length === 0) {
          return {
            content: [{ type: 'text', text: 'No voices configured — set the VOICE_MAP secret (JSON of voice name → ElevenLabs voice ID) via `wrangler secret put VOICE_MAP`.' }],
          };
        }
        const voiceName = (args.voice || voiceNames[0]).toLowerCase();
        const voiceId = voices[voiceName] || voices[voiceNames[0]];
